...
def create_plugin(slug, plugin_dir=None):
    ...
    if not is_valid_slug(slug):
        raise UserException(
            "Slug must only contain ASCII letters, numbers, '-', and '_'."
        )
...
"""
    ...
    eprint(
        f"You may use `make`, `make clean`, `make dist`, `make install`, etc in the {plugin_dir} directory."
    )
...
def create_manifest(slug, plugin_dir="."):
    ...
    manifest["license"] = input_default(
        "License (if open-source, use license identifier from https://spdx.org/licenses/)",
        manifest.get("license", "proprietary"),
    )
    manifest["brand"] = input_default(
        "Brand (prefix for all module names)", manifest.get("brand", manifest["name"])
    )
    manifest["author"] = input_default("Author", manifest.get("author", ""))
    manifest["authorEmail"] = input_default(
        "Author email (optional)", manifest.get("authorEmail", "")
    )
    manifest["authorUrl"] = input_default(
        "Author website URL (optional)", manifest.get("authorUrl", "")
    )
    manifest["pluginUrl"] = input_default(
        "Plugin website URL (optional)", manifest.get("pluginUrl", "")
    )
    manifest["manualUrl"] = input_default(
        "Manual website URL (optional)", manifest.get("manualUrl", "")
    )
    manifest["sourceUrl"] = input_default(
        "Source code URL (optional)", manifest.get("sourceUrl", "")
    )
    manifest["donateUrl"] = input_default(
        "Donate URL (optional)", manifest.get("donateUrl", "")
    )
...
def create_module(slug, panel_filename=None, source_filename=None):
    ...
    if module_manifest:
        eprint(
            f"Module {slug} already exists in plugin.json. Edit this file to modify the module manifest."
        )
        ...
        module_manifest["description"] = input_default(
    ...
    if panel_filename:
        ...
        if source_filename and os.path.exists(source_filename):
            if (
                input_default(
                    f"{source_filename} already exists. Overwrite? (y/n)", "n"
                ).lower()
                != "y"
            ):
                return
...
def panel_to_components(tree):
    ...
    for el in group:
        ...
        if el.tag == "{" + ns["svg"] + "}rect":
            ...
            el.tag == "{" + ns["svg"] + "}circle"
            or el.tag == "{" + ns["svg"] + "}ellipse"
        ):
