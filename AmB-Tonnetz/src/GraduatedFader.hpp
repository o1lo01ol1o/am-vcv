...
template <int H>
struct GraduatedFader : app::SliderKnob,
                        baconpaul::rackplugs::StyleParticipant {
  ...
  GraduatedFader() {
    ...
    handle = new BufferedDrawFunctionWidget(
        Vec(0, 0), Vec(20, 41), [this](auto vg) { drawHandle(vg); });
    fb->addChild(handle);
  }
  ...
  void onChange(const event::Change &e) override {
    if (getParamQuantity()) {
      ...
      handle->box.pos =
          math::Vec(math::rescale(v, 0.f, 1.f, minHandlePos.x, maxHandlePos.x),
  ...
  void drawHandle(NVGcontext *vg) {
    ...
    NVGpaint vgr =
        nvgLinearGradient(vg, 0, 0, 0, 19, nvgRGBA(170, 170, 190, 255),
  ...
  void drawBackground(NVGcontext *vg) {
    ...
    for (int i = 0; i <= nStrokes; ++i) {
      ...
      nvgStrokeColor(
          vg, style->getColor(baconpaul::rackplugs::BaconStyle::SLIDER_NOTCH));
      nvgStrokeWidth(vg, 0.5);
      nvgStroke(vg);
    }
    ...
    nvgFillColor(
        vg, style->getColor(baconpaul::rackplugs::BaconStyle::SLIDER_SLOT));
    nvgFill(vg);
  }
