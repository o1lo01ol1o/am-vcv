...
struct BufferedDrawFunctionWidget
    : virtual FramebufferWidget /* widget::Widget*/
{
...
#if DEBUG_MEM
    memDebugger["bdw"]++;
    std::cout << "CTOR >> Outstanding BDWs = " << memDebugger["bdw"]
              << std::endl;
#endif
  }
  ...
  ~BufferedDrawFunctionWidget() override {
    memDebugger["bdw"]--;
    std::cout << "DTOR << Outstanding BDWs = " << memDebugger["bdw"]
              << std::endl;
  }
#endif
};
...
struct BufferedDrawFunctionWidgetOnLayer : BufferedDrawFunctionWidget {
  int layer{1};
  BufferedDrawFunctionWidgetOnLayer(rack::Vec pos, rack::Vec sz, drawfn_t draw_,
                                    int ly = 1)
